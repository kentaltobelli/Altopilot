...
// By arduino.cc user "Krodal" and supplemented by Kent Altobelli
// June 2012
...
#define MPU6050_I2C_ADDRESS 0x68  // Default MPU6050 I2C address
#define FILTER_COEF .98  // Gryoscope coefficient for complementary filter
#define FILTER_COEF_COMP 1-FILTER_COEF  // Complement of FILTER_COEF
#define RAD2DEG 57.29578

#define GYRO_SENS 32.77  //2^16 / range(+/-1000) = 2^16/2000 = 32.77
#define ACCEL_SENS 8192  //2^16 / range(+/-4G) = 2^16/8 = 8192
#define GYRO_SENS_1000000_INVERSE 1/(GYRO_SENS * 1000000) // Multiply by gyro and dt(us) to get angle


// Structure to hold data for all axes
struct TAxisData {
  int gyro;
  int gyro_offset;
  int typical_gyro_offset;
  long accel;
  float accel_angle;
  float angle;
  float last_angle;
...
// Array of structures for all 3 axes
extern TAxisData x_axis, y_axis, z_axis;
extern TAxisData axis_ptr[];


class CMPU6050 {
public:
  CMPU6050();  // Class constructor
  ~CMPU6050();  // Class destructor

  int Initialize();
  int UpdateAngle(uint16_t dt);
...
private:
  ...
  // Structure to hold data for all axes
   ...
   TAxisData* axis_ptr[3];
   */
  int GetData();  // Reads data from MPU
  void IntegrateGyro(uint16_t dt);  // Updates angle using gyro data
  void ComplementAccel();  // Updates angle using accel data
  void SwapBytes(uint8_t* x, uint8_t* y);
};
...
#endif //if not defined _MPU6050_H_
